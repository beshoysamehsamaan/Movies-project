...
namespace Get_Movies.Models
{
    public class ViewLog
    {
        ...
        private IQueryable<ViewLog> SearchAnd(Boolean exactStringMatching)
        {
            ...
            if (!String.IsNullOrWhiteSpace(this.Time_Stamp.ToString().ToString())) { resultSet = exactStringMatching ? ((resultSet ?? entity).Where(vl => vl.Time_Stamp.ToString().Equals(this.Time_Stamp.ToString()))) : ((resultSet ?? entity).Where(vl => vl.Time_Stamp.ToString().Contains(this.Time_Stamp.ToString()))); }
            return resultSet;
        }
